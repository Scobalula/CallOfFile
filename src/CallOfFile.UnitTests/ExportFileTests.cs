...
namespace CallOfFile.UnitTests
{
    ...
    {
        public static string TestFiles { get; } = Path.Combine(Environment.GetEnvironmentVariable("COF_TEST_FILES_DIR") ?? string.Empty, "ExportTests");

        public static string OutputFiles { get; } = Path.Combine(Environment.GetEnvironmentVariable("COF_TEST_FILES_DIR") ?? string.Empty, "ExportTestsResults");
        ...
        public void TestBinaryFiles()
        {
            ...
            foreach (var file in Directory.EnumerateFiles(TestFiles, "*.*_export"))
            {
            ...
            foreach (var file in Directory.EnumerateFiles(OutputFiles, "*.*_bin"))
            {
